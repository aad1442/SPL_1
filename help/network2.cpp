...
// Function to parse network connections for TCP
std::vector<NetworkConnection> parse_tcp_connections()
{
...
// Function to parse network connections for UDP
std::vector<NetworkConnection> parse_udp_connections()
{
    std::vector<NetworkConnection> connections;
    std::ifstream netstat_output("/proc/net/udp");

    if (!netstat_output)
    {
        std::cerr << "Failed to open /proc/net/udp" << std::endl;
        return connections;
    }
    ...
    while (std::getline(netstat_output, line))
    {
        ...
        // UDP connections are always considered "ESTABLISHED" in /proc/net/udp
        connections.push_back(connection);
    }
...
int main()
{
    ...
    // Parse network connections for both TCP and UDP
    std::vector<NetworkConnection> tcpConnections = parse_tcp_connections();
    std::vector<NetworkConnection> udpConnections = parse_udp_connections();

    // Combine TCP and UDP connections into a single vector
    std::vector<NetworkConnection> allConnections;
    allConnections.insert(allConnections.end(), tcpConnections.begin(), tcpConnections.end());
    allConnections.insert(allConnections.end(), udpConnections.begin(), udpConnections.end());

    // Log network connections to the file
    log_network_connections(allConnections, filename);
